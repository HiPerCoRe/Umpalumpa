...
namespace fourier_processing {
  ...
  namespace {// to avoid poluting
    ...
    struct Strategy1 : public FP_CUDA::Strategy
    {
      ...
      // But I would say that absolute path into the project root is better option (makes more sense
      // when including headers in .cu)
        ...
        ktt::Tuner &tuner) override final
      {
        ...
        if (canProcess) {
          ...
          tuner.AddParameter(
            kernelData.kernelId, "applyFilter", std::vector<uint64_t>{ s.GetApplyFilter() });
          tuner.AddParameter(
            kernelData.kernelId, "normalize", std::vector<uint64_t>{ s.GetNormalize() });
        ...
        ktt::Tuner &tuner) override final
      {
        ...
        //  out.data.info.GetSize().total,
        //  ktt::ArgumentAccessType::WriteOnly,// FIXME these information should be stored in the
        //  physical descriptor ktt::ArgumentMemoryLocation::Unified);// ^
        ...
        auto filter = tuner.AddArgumentScalar(
          size_t(0));// instead of nullptr, KTT is not happy with pointers in scalar arguments
        auto normFactor =
          tuner.AddArgumentScalar(1.f / static_cast<float>(in.data.info.GetSize().single));

        tuner.SetArguments(
          kernelData.definitionIds.front(), { argIn, argOut, inSize, outSize, filter, normFactor });
