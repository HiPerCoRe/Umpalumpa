...
#ifndef __CUDACC__
#include <cmath>// for fmaf and others
#endif

namespace umpalumpa::fourier_reconstruction {

template<typename T, typename U> inline CUDA_HD static U clamp(U val, T min, T max)
{
...
template<typename T> CUDA_HD void multiply(const T transform[3][3], data::Point3D<T> &inOut)
{
    ...
    ans =
      ...
      + y
          * (3.5156229f
             + y
                 * (3.0899424f
                    + y * (1.2067492f + y * (0.2659732f + y * (0.360768e-1f + y * 0.45813e-2f)))));
  } else {
    y = 3.75f / ax;
    ans =
      (expf(ax) * (1 / sqrtf(ax)))
