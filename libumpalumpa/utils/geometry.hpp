...
namespace utils {
  ...
  CUDA_HD
  template<typename T> T getZ(T x, T y, const data::Point3D<T> &n, const data::Point3D<T> &p0)
  {
  ...
  CUDA_HD
  template<typename T> T getY(T x, T z, const data::Point3D<T> &n, const data::Point3D<T> &p0)
  {
    // from a(x-x0)+b(y-y0)+c(z-z0)=0
  ...
  CUDA_HD
  template<typename T> T getX(T y, T z, const data::Point3D<T> &n, const data::Point3D<T> &p0)
  {
    // from a(x-x0)+b(y-y0)+c(z-z0)=0
