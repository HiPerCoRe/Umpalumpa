...
#include <libumpalumpa/operations/fourier_reconstruction/fr_starpu_kernels.hpp>
...
namespace {// to avoid poluting
  ...
  void CpuInit(void *args)
  {
    ...
    auto *op = reinterpret_cast<FRCPU *>(a->ops.at(id));
    if (nullptr == op) { op = new FRCPU(); }
  ...
  void CudaInit(void *args)
  {
    auto *a = reinterpret_cast<Args *>(args);
    auto id = static_cast<size_t>(starpu_worker_get_id());
    auto *op = reinterpret_cast<FRCUDA *>(a->ops.at(id));
    if (nullptr == op) {
      std::vector<CUstream> stream = { starpu_cuda_get_local_stream() };
      op = new FRCUDA(static_cast<int>(id), stream);
    }
    if (!op->Init(a->out, a->in, a->settings)) {
      delete op;
      op = nullptr;
...
FRStarPU::~FRStarPU()
{
  ...
  Cleanup();
  starpu_execute_on_each_worker(DeleteOp<FRCPU>, &ops, STARPU_CPU);
  starpu_execute_on_each_worker(DeleteOp<FRCUDA>, &ops, STARPU_CUDA);
}
...
void FRStarPU::Cleanup()
{
  if (!this->IsInitialized()) return;
  Synchronize();
  starpu_execute_on_each_worker(UniversalCleanup<FRCPU>, &ops, STARPU_CPU);
  starpu_execute_on_each_worker(UniversalCleanup<FRCUDA>, &ops, STARPU_CUDA);
  AFR::Cleanup();
}
