#include <libumpalumpa/algorithms/initialization/starpu.hpp>
...
namespace {// to avoid poluting
  ...
  struct CodeletArgs
  {
    ...
    data::Payload<data::LogicalDescriptor> val;
    std::vector<Abstract *> *algs;
  };
  ...
  void Codelet(void *buffers[], void *func_arg)
  {
    ...
    auto pVal = StarPUUtils::Assemble(args->val, buffers[2]);
    auto in = Abstract::InputData(pIn, pVal);
...
bool StarPU::ExecuteImpl(const OutputData &out, const InputData &in)
{
  ...
  auto CreateArgs = [this, &out, &in]() {
    ...
    memcpy(reinterpret_cast<void *>(&a->val), &in.GetValue(), sizeof(a->val));
    return a;
  };
  ...
  task->handles[2] = *StarPUUtils::GetHandle(in.GetValue().dataInfo);
  ...
  task->cl = [] {
    ...
    c.model = [] {
      ...
      m.symbol = "Initialization_StarPU";
      return &m;
    }();
    return &c;
  }();
