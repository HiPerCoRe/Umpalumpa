...
namespace data {
  ...
  public:
    ...
    inline const auto &GetSpatialSize() const {
      return size;
    }

    inline const auto &GetPaddedSpatialSize() const {
      return paddedSize;
    }

    inline const auto &GetFrequencySize() const {
      return frequencyDomainSize;
    }

    inline const auto &GetPaddedFrequencySize() const {
  ...
  // fixme say that padded size is size + padding
    ...
    virtual FourierDescriptor Subset(size_t &safeCount, const size_t startN, const size_t count) const
    {
      ...
      return FourierDescriptor(size.CopyFor(safeCount), GetPaddedSize().CopyFor(safeCount));
    }
    ...
    virtual size_t Offset(size_t x, size_t y, size_t z, size_t n) const
    {
      ...
      assert(GetSize().z >= z);
      assert(GetSize().n >= n);
      return (n * GetPaddedSize().single + z * (GetPaddedSize().x * GetPaddedSize().y) + y * (GetPaddedSize().x) + x);
    }
