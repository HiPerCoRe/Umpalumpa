...
class FFT_Tests : public TestOp<AFFT>
{
protected:
    ...
    size_t batchSize = 0)
  {
    ...
    if (0 == batchSize) {
      ASSERT_TRUE(op.Init(out, in, forwardSettings));
      ASSERT_TRUE(op.Execute(out, in));
    } else {
      ...
      //   if (0 == offset) { ASSERT_TRUE(op.Init(tmpOut, tmpIn, forwardSettings)); }
      //   ASSERT_TRUE(op.Execute(tmpOut, tmpIn));
      // }
    }

    auto inverseSettings = forwardSettings.CreateInverse();
    if (0 == batchSize) {
      ASSERT_TRUE(op.Init(inverseOut, inverseIn, inverseSettings));
      ASSERT_TRUE(op.Execute(inverseOut, inverseIn));
    } else {
      GTEST_SKIP() << "Batching is not yet supported";
      // for (size_t offset = 0; offset < in.GetData().info.GetSize().n; offset += batchSize) {
      ...
      //   if (0 == offset) { ASSERT_TRUE(op.Init(tmpOut, tmpIn, inverseSettings)); }
      //   ASSERT_TRUE(op.Execute(tmpOut, tmpIn));
      // }
    }
    // wait till the work is done
