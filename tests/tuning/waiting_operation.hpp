...
namespace {
  ...
  struct WaitingStrategy : public WaitingOperation::KTTStrategy
  {
    using WaitingOperation::KTTStrategy::KTTStrategy;
    ...
    bool Execute(const WaitingOperation::OutputData &,
      const WaitingOperation::InputData &) override final
    {
      for (int i = 0; i < GetSettings().numberOfKernels; i++) {
...
std::vector<std::unique_ptr<WaitingOperation::Strategy>> WaitingOperation::GetStrategies() const
{
  std::vector<std::unique_ptr<WaitingOperation::Strategy>> vec;
